...
[테이블] : N개의 카드에 대한 각각의 카드 비용을 생각해 가능한 경우의 수 중 총합 금액의 최대값을 출력한다
...
dp[3] = dp[2] + arr[1] = 5(최대값) + 1 = 6
dp[3] = dp[1] + arr[2] = 1 + 5 = 6
dp[3] = dp[0] + arr[3] = 0 + 6 = 6

dp[4] = dp[3] + arr[1] = 6(최대값) + 1 = 7
dp[4] = dp[2] + arr[2] = 5(최대값) + 5 = 10
dp[4] = dp[1] + arr[3] = 1 + 6 = 7
dp[4] = dp[0] + arr[4] = 0 + 7 = 7

dp[i] = dp[i-j] + arr[j]
점화식은 이렇게 나오고 진행되는 과정 중 작은 값도 나올 수 있는 경우의 수가 있는데 계속해서 큰 값으로 갱신을 해야하기 때문에 Math.max를 사용해야 한다
